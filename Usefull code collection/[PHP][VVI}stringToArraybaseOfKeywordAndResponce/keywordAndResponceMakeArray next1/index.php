...
<?php
	...
	$string = "keyword1|aaa [bbb|ccc e] ddd eee [fff|ggg|www] [fff|ggg|www|1223] hhh [bbb|ccc e]";
...
for ($i=0; $i < $loop ; $i++) { 
	...
	// 4) then each under bracket value replace into ^ (exm: aa|b^c )  And finally Full respance under bracket value is replaced by ^ ex:aaa [bbb|ccc^e] ddd eee [fff|ggg|www] [fff|ggg|www|1223] hhh [bbb|ccc^e]
	$stringRepFromMainString = str_replace($optiionReal,$underScoreReplace,$response);
	// echo $underScoreReplace."<br>";
}

echo "Finel responce: ".$stringRepFromMainString;



	if(preg_match_all("/\\[(.*?)\\]/", $stringRepFromMainString, $matches))
	{

		// 5)Then String to arry convert using explode(' ',$responce) function
		$strToarray = explode(' ',$stringRepFromMainString);
		echo "<pre>";
		print_r($strToarray);
		echo "<br>";
